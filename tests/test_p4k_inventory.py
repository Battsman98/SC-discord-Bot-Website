...
def test_p4k_catalog_uses_real_type_when_subtype_is_undefined() -> None:
    catalog = P4KInventoryCatalog(Path("data/p4k_inventory_snapshot.json"))

    blizzard = catalog.lookup("Blizzard")[0]
    assert blizzard.name == "Blizzard"
    assert blizzard.section == "Cooler"
    assert blizzard.size == "3"
