...
The Hub also contains a read-only `member-applications` embed with an Apply button. Visitors answer two private button-based questions and enter their RSI handle in a private third-step text modal. Completed applications are posted to `membership-application-reviews`, which only the server owner and bot can access. Approving an application replaces the applicant's `Visitor` role with the server's existing `Members` role; denying it leaves their roles unchanged. The bot never creates a replacement membership role.
